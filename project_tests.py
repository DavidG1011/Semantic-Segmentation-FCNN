...
def test_optimize(optimize):
    ...
    logits, train_op, cross_entropy_loss, iou_obj = optimize(layers_output, correct_label, learning_rate, num_classes)
...
def test_train_nn(train_nn):
    ...
    iou_obj = tf.placeholder(tf.float32, name='iou_obj')
    with tf.Session() as sess:
        parameters = {
            'sess': sess,
            'epochs': epochs,
